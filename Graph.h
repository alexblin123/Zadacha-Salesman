...
void getAnswer(int*** mat, int n, int** help, int* path)
{
	for (int l = 0; l < n; l++)
	{
		for (int i = 0; i < n; i++)
...
void answerPrepare(int***& mat, int& n, int**& help, int*& result) // подготовка результата с помощью подмассивов 
{
	n = amountVerts;
...
void answerOutPut(int*** mat, int n, int** help, int* result) // вывод ответа
{
	answerPrepare(mat, n, help, result);

	int s = 0;

	getAnswer(mat, n, help, result);
...
template<class T>
int Graph<T>::GetAmountVerts() { return this->vertList.size(); }

template<class T>
bool Graph<T>::IsEmpty() { return this->vertList.size() == 0; }

template<class T>
bool Graph<T>::IsFull() { return this->vertList.size() == maxSize; }
...
Graph<int> makeGraph()
{
	...
	cout << "Количество вершин: "; cin >> amountVerts; cout << endl; 
	cout << "Количество рёбер: "; cin >> amountEdges; cout << endl; 
...
void Graph<T>::InsertEdge(const T& vertex1, const T& vertex2, int weight)
{
	if (this->GetVertPos(vertex1) != (-1) && this->GetVertPos(vertex2) != (-1))
	{
		...
		if (this->adjMatrix[vertPos1][vertPos2] != 0 && this->adjMatrix[vertPos2][vertPos1] != 0)
		{
			...
			cout << "Такое ребро уже есть." << endl;
			return;
		}
	...
	{
		cout << "Таких вершин нет в графе." << endl;
		return;
	}
}
...
void drawVertex(int n)
{
	for (int i = 0; i < n; i++)
	{
		drawCircle(vert[i].x, vert[i].y, R);
		drawText(i + 1, vert[i].x, vert[i].y);
	}
}
...
void Graph<T>::DrawGraph()
{
	...
	for (int i = 0; i < n; i++)
	{
		for (int j = i + 1; j < n; j++)
		{
			...
			if (a != 0)
			{
				drawEdge(a, vert[i].x, vert[i].y, vert[j].x, vert[j].y);
			}
		}
	}
	drawVertex(n);
...
void mouseClick(int btn, int stat, int x, int y) {
	...
	if (stat == GLUT_DOWN) {
		...
		{
			answerOutPut(mat, n, help, result);
		}
...
void display()
{
	...
	glClearColor(0.400, 0.804, 0.667, 0.0);
